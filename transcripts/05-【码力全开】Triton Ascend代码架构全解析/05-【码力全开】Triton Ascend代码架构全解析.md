...
我们这边首先拿了一个Program ID，PID，Program ID是根据用户输入的切分来决定的，所以如果用户选择了切分为4的话，实际上我们会有4个这样的Kernel，每一个Kernel的ID，就是代表你在调用着第几个Kernel。
...
这边的数值就和我们的圆满数值是一对应的后续在Pathom代码里面，会看到这里有TL的ProgramID，这个其实也会变成我们的TritonLanguage的MIR，可以从我TT.getProgramID。

后续我们把这个ProgramID成为block size，所以你看到这里有个arif.multiplication，这里是i,是代表这是integer，后续也可以看到这里mask，offset所为N of n elements。

所以这边也做了一个比较，是个less than的一个比较，后续也可以看到这边的圆满是一对应的例如这里的TL.arrange，它最后会变成我们的Triton方言的Mixed Arrange，这里加法就变成一个add pointer。
...
所以在这个过程里面，cpu会往npu发射一个指令，之后把相关的讯息发进去，所以第一次launch一个kernel的时候，会发生以下的步骤，第一步，这是进行我们python的前端编译，trilang会将我们的pythonkernel。

编译成我们的TTIR，这个过程里面，我们会展开一些循环，也会处理一些长量，后续我们这边也会进行一些优化，以及一些代码生成，最后这个TTIR，经过我们的trilangascent，经过我们的ascentpuir。

经过我们的businessir，最后变成lvm，最后生成出能在目前的npu上，能跑的汇变代码，由于trilangascent，it's just in time compilation，所以生成出来的一个二进制。

会变缓程，放在这个trilangcache里面，下次如果再进行调用，使用着一样的参数，一样的block size，一样的圆码，那么当我们启动kernel的时候，我们就只需要直接加载完成，不需要重新去编译。
...
第一个就是autoblockify，这个pass它能够把多个逻辑盒合并成一个逻辑盒，来减少我们的开销，第二个是triton destructure，以前这个叫做triton linearizer，这个pass就是透过分析我们的low store。
...
假设我们的grid分别是5,3,2,x为5,y为3,z为2，那么它总共的逻辑盒数量就是5乘3乘2等于30，假设用户要把autoblockify v2的size设定为7，那么这就代表每7个逻辑盒要并在一起。

所以blockify后的第0个逻辑盒，它要执行的就是本来index0到6的逻辑盒运算，同样的道理，blockify后的逻辑盒1，它就会执行原kernel的7,8,9,10,11,12,13，如此类推，直到去到我们的blockify后的第4个逻辑盒。

由于我们只有5乘3乘2，我们只有30个逻辑盒，所以第4个逻辑盒它只会执行我们的index28跟index29的逻辑盒运算，后续的后续就不会参与运算了，所以可以看到本来我们要执行30个逻辑盒，透过autoblockify。

我们把逻辑盒的数量直接除以7，剩下5个，这样的话我们每次启用我们的好代码的时候，我们就可以减少我们的开销，但是在我们改了他们的切分逻辑之后，有些运算其实我们都是要修改的举个例子，如果我们本来的代码里面我们需要感知到我们的program id。
...
所以这里的数字就会是0,0,0,0,0,0,0,1，5个0,1个1，后续再去做个mod x，因为他们都是0跟1，所以就mod x之后，还是这里5个0,1个1，所以在blockify后的第0个逻辑盒，它前面这5个是处于我们的pidx0的这里就是pidx1。

同样的道理，这里的blockify1，blockify第一个逻辑盒，它的index1逻辑盒，前面这里就是xid为1，这两个就是xid为2，如此类推，我们就可以正确的把我们的kernel，在转化的过程里面。
...
假设我们这个手轴64，它的stripe是不固定的而维度的stripe是固定的话，那么我们要怎么去处理这个，这个前面是离散的右面是右右边是连续的纺承呢由于我们NPO硬件，它是不支撑任何的离散纺承的我们是需要把我们的手轴进行打散。

所以你大家可以看到，这里的64，我们会变成我们的control flow，透过我们的force循环，从我们的c0到我们的c64，每次增加1，把这个，这个iteration argument，传入到我们的extract size里面。

这样的话我们手轴就可以，变成我们的离散纺承，一个一个来去访问了而由于我们的维度它是128，这个是一个固定的stripe，所以呢extract size也可以把这个128直接放在这里，这样就是我们怎么去进行离散纺承的改写了。
...
我每次point都是load，或者store我的index1，这样的话，它的stripe就是0，后续就是我们的规整轴，规整轴的定义就是我们的stripe，要是固定的就好像刚刚的例子，我把0到3，的一个矩阵。

把它乘以2，0246，它的stripe是2，所以这里还是一个规整的离散绑成离散轴，就正如我刚所描述的0149，他们stripe是不固定的这样的话就是跟离散轴，我们做arrange的时候，只需要输入，我们开始的index1。

跟结束的index1，或者有时候也不是要输入开始的index1，我们就可以得出一个规整的一个数字，可能从0到100，这里就stripe是1，所以这个是一个规整轴，后续，到了我的加法跟减法操作，假设是两个标量加起来。
...
假设我们是零部负轴跟规整轴呢假设想一下我们的元素，我们有一个3*3的矩阵，第一行是345，第二行也是345，第三行也是345，从左到右，345，它就是一个规整轴，它的stripe是1，这里的零部副轴，就是从上老下。

就是333，444，以及555，这种情况下，如果我想要对我们的零部副轴，来去做一个reduce，我把333做一个乘法，变成27，我把444做个乘法，变成64，我把555做个乘法，变成125，我们从上往下。

333乘在一起，就剩下一个size，剩下一个27了44变成64，555变成125，这里的第一根轴，就是标样轴，但是我们第二根轴，从左往右，从345，stripe为1，变成27，64，125，他们的stripe。
...
它是一个面向我们生存平台，构建的一个triton编译框架，我的目的就是让我们triton代码，能够在我们的生存硬件上高效运行，其中主要经过的编译流程有四个，第一个就是我们triton python dsl。

会把我们的圆码翻译成我们tritir，第二个就是把我们tritir，转换成我们tritadapterir，tritadapterir就是具有我们上游社区各种的方言，来让我们的ascent mpyr比较容易对接。
...
也能保持到我们自定义算子的私有性，这个也增加了我们Triton编程里面的生存亲和，确保我们写出的算子是在昇腾上有一个很好的性能的，所以这边就是我们Triton Ascend的一个总结，我们目前Trit。
